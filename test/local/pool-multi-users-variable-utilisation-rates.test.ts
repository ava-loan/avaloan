...
describe('Pool with fixed interests rates', () => {
  ...
  describe('Deposit, borrow, wait & borrow more', () => {
    ...
    it("user4 repays part of loan, user1 withdraws all deposit", async () => {
      ...
      expect(fromWei(await pool.getDepositRate())).to.be.closeTo( 0.9019303447327899, 0.000001);
      expect(fromWei(await pool.getBorrowingRate())).to.be.closeTo(0.8470967584828037, 0.000001);
    ...
    it("should accumulate interests for another year", async () => {
      ...
      expect(depositUser1).to.be.closeTo( 0, 0.000001);
      expect(depositUser3).to.be.closeTo(4.114142243435112, 0.000001);
      expect(borrowedUser2).to.be.closeTo(0, 0.000001);
      expect(borrowedUser4).to.be.closeTo(2.48387315725814, 0.000001);
